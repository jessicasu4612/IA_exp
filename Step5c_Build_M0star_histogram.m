...
rng(2);
addpath('.\Library\');
result_path = '.\Results\M0star\histogram\';
...
for attackerCount = 1: size(attacker_list, 1)
    ...
    featureIndex = [1:49];
    [CWF] = xlsread(['.\Data\featureDistribution\tempSelected.xlsx'], attackerName);
    for index = 1:size(CWF,2)
        featureIndex(featureIndex == CWF(1,index)) = [];
    end
